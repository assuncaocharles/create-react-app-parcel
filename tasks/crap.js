...
const scriptsFileName = cp
  .execSync(`npm pack`, { cwd: path.join(packagesDir, 'react-scripts-parcel') })
  .toString()
  .trim();
const scriptsPath = path.join(packagesDir, 'react-scripts-parcel', scriptsFileName);
...
const craScriptPath = path.join(packagesDir, 'create-react-app-parcel', 'index.js');
cp.execSync(`node ${craScriptPath} --scripts-version="${scriptsPath}" ${args.join(' ')}`, {
  cwd: rootDir,
  stdio: 'inherit'
});
